...
from Networks.Networks import PHENUNet, OnlyCNN, PHENCNN, OnlyUNet
from train_phen import get_loss_fn, compute_metrics, GasFieldDatasetPre
...
def main(cfg, gpu_id=None):
    ...
    # 5. model init
    if model_type == "phenunet":
        model = PHENUNet(in_ch=3, out_ch=1,
        ...
        model = OnlyCNN(in_ch=3, out_ch=1,
                        base_ch=base_ch, depth=depth).to(device)
    elif model_type == "phencnn":
        model = PHENCNN(in_ch=3, out_ch=1,
                        base_ch=base_ch, fuse_dim=fuse_dim, depth=depth).to(device)
    else:
